...
def select_face(im, r=10, source=0):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--frame_end', required=True, help='the number of the last frame')
    parser.add_argument('--output_folder', required=True, help='name of output folder')
    parser.add_argument('--input_folder', required=True, help='name of input folder')
 
    args = parser.parse_args()
    src = args.input_folder+'/video'
    out1 = args.output_folder+'/video'
    end = int(args.frame_end)+1
    for x1 in range(1,end):
    	# Read images
    	src_img = cv2.imread(args.dst)
    	dst_img = cv2.imread(src+str(x1)+'.jpg')

    	# Select src face
    	src_points, src_shape, src_face = select_face(src_img, source=0)
    	# Select dst face
    	dst_points, dst_shape, dst_face = select_face(dst_img, source=1)
    	...
    	dir_path = os.path.dirname(out1+str(x1)+'.jpg')
    	if not os.path.isdir(dir_path):
        	os.makedirs(dir_path)

    	cv2.imwrite(out1+str(x1)+'.jpg', output)
    	##For debug
